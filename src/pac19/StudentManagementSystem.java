...
public record Student(String fullName, double averageScore) {
    @Override
...
public class StudentManagementSystem {
    ...
    public void searchStudent(String fullName) throws StudentNotFoundException {
        for (int i = 0; i < numStudents; i++) {
            if (students[i].fullName().equalsIgnoreCase(fullName)) {
    ...
    public void sortByAverageScore() {
        Arrays.sort(students, 0, numStudents, (s1, s2) -> Double.compare(s2.averageScore(), s1.averageScore()));
    }
